...
- suite variables are cloned and then extended with the test's, so test keys win;
  values are converted from TOML to JSON so the runner can hand them to Rhai
...
1. Builds a `reqwest::blocking::Client`, applying the timeout if there is one,
   and, unless the test sets `cookies = false`, a fresh `CookieJar` as its
   cookie provider.
2. Builds a fresh `rhai::Engine` and runs all six `register_*` functions
...
Six registration functions, called in the same order by both `execute_once()`
and `check`'s `compile()`:

| Function | Registers |
| --- | --- |
| `register_http` | `Request`/`Response` types, the six verbs, builder methods, `send`, response accessors, `field`, `basic` |
| `register_cookies` | `cookie(url, name)`, `cookies`, `set_cookie`, `clear_cookies`, all against the per-test jar |
| `register_debug` | `on_print` / `on_debug` hooks, `print_response` and `print_cookies`, all writing into the sink |
...
`CookieJar` wraps a `cookie_store::CookieStore` in an `RwLock` and implements
reqwest's `CookieStore` trait itself. It does not use `reqwest::cookie::Jar`,
because `Jar` can only report what it would send to a single URL, and
`print_cookies()` needs to list the whole store. `check` passes `None`
for the jar, so the functions still resolve there.
