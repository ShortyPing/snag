...
`CookieJar` wraps `reqwest::cookie::Jar` in an `RwLock`. The wrapper exists
because `Jar` has no clear method and the client holds its `Arc`, so
`clear_cookies()` swaps in a new `Jar` behind the lock. `check` passes `None`
