...
1. Checks the script exists, then reads it.
2. Builds a `reqwest::blocking::Client`, applying the timeout if there is one.
3. Builds a fresh `rhai::Engine` and registers HTTP, debug, assertion, and env
   functions.
4. Installs an `on_progress` hook when a timeout applies: every
   `PROGRESS_INTERVAL` (2,000) operations it compares `Instant::now()` against
   the deadline and returns `Some(Dynamic::UNIT)` to abort.
5. Pushes the test's variables into a `Scope` as **constants**.
6. Runs the script, and passes any error through `classify()`.
...
Four registration functions, called in the same order by both `execute_once()`
and `check`'s `compile()`:
