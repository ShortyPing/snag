...
Variables are `BTreeMap<String, toml::Value>` on the manifest side and
`BTreeMap<String, serde_json::Value>` on the resolved `Test`, converted by
`to_json()`. The detour through JSON exists because the runner hands values to
Rhai via serde, and a TOML datetime serialises as a private struct that would
otherwise surface in scripts as a one-key map; `to_json()` turns it into its
RFC 3339 string instead.
...
| `ReqBuilder` | Method, URL, headers, per-request cookies, optional body, a cloned client and the jar |
| `CookieJar` | `RwLock<cookie_store::CookieStore>` implementing reqwest's `CookieStore`; not `Jar`, which cannot list its contents |
| `Response` | Status, text, headers, duration |
| `register_http()` | Verbs, builder methods, `send`, accessors, `field`, `basic` |
| `register_cookies()` | `cookie`, `cookies`, `set_cookie`, `clear_cookies`; `None` jar means `cookies = false` |
| `register_debug()` | `on_print`, `on_debug`, `print_response`, `print_cookies` |
...
| `set_cookie_value()` / `merge_cookie_header()` | Back `res.cookie()` and `.cookie(k, v)` |
...
| `base64` | `basic()` |
| `cookie_store` | Backs `CookieJar`; the same version reqwest's `cookies` feature pulls in |

## `src/revision.rs`

Builds the release manifest. See [revision.json](../reference/revision-file.mdx).
...
`snag update` and the startup notice. See [updating](../guides/updating.mdx).
