...
| `cookies` | bool | `true` | `false` gives each test no [cookie jar](./script-api.mdx#cookies) |
| `variables` | table of string → [any TOML value](#variables) | `{}` | Variables shared by every test in the file |
...
| `cookies` | bool | no | the suite `cookies` | Turns this test's cookie jar on or off |
| `variables` | table of string → [any TOML value](#variables) | no | `{}` | Overrides of suite variables, for this test only |
...
`[variables]` and `[test.variables]` accept any TOML value. Types are preserved
on the way into the script:

| TOML | In the script |
| --- | --- |
| string | string |
| integer | integer |
| float | float (`inf` and `nan` are rejected at load time) |
| boolean | bool |
| array | array |
| table | object map |
| datetime | string, in RFC 3339 form |
...
[variables.payload]          # a sub-table becomes a map
project = "snag"
ok = true
```
...
let res = post(`${base_url}/items`).json(payload).send();
assert_eq(payload.project, "snag");
assert_eq(field(payload, "ok"), true);
```

A test overrides a sub-table with `[test.variables.payload]`. Values arrive as
constants, so `payload.ok = false` fails the same way reassigning `payload`
would.
...
**Variables** — a test's key wins over the suite's key. Per key, no deep
merge: a test-level table replaces the suite's table under that key as a whole.
...
**Cookies** — the test's `cookies` wins over the suite's. If neither is set, the
test gets a jar:

```
true  →  suite cookies  →  test cookies
```
...
| `cookies` | test `cookies`, else suite `cookies`, else `true` |
| `script` | `file`, joined to the manifest's directory and normalised |
| `vars` | suite `variables` merged with test `variables`, TOML types preserved |
