...
- `runner.rs` — the whole execution model. `run()` partitions on `parallel_safe`: `false` tests run first in a serial batch, the rest share a worker pool. `execute_batch()` spawns scoped threads pulling from an `AtomicUsize` index and sends `Event`s back over an `mpsc` channel, so **the reporter only ever runs on the main thread** — no locking, and reporters need not be `Sync`. A shared `cancel` flag drains the remainder as `Skipped` (fail-fast, or a reporter error). Each test gets a fresh `Engine`, a fresh `reqwest::blocking::Client`, a fresh `CookieJar` (unless `cookies = false`), and a fresh output sink per attempt.
